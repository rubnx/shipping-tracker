...
class SmartContainerRouter {
     ...
     */
    detectCarrierFromFormat(trackingNumber) {
     ...
     */
    prioritizeProviders(availableProviders, context, carrierDetection) {
     ...
     */
    calculateProviderScore(provider, context, carrierDetection) {
        let score = 0;
        ...
        if (context.userTier === 'free') {
            if (cost === 0)
                score += 100; // Big bonus for free providers
        }
     ...
     */
    determineFallbackStrategy(context) {
     ...
     */
    getProviderStatsIncludingUnknown(provider) {
        const failures = this.failureHistory.get(provider);
        return {
            provider,
            cost: this.providerCosts.get(provider) || 0,
     ...
     */
    applyHeuristicDetection(trackingNumber) {
        const prefixMap = {
     ...
     */
    generateReasoningExplanation(context, carrierDetection, prioritizedProviders) {
        const reasons = [];
...
exports.SmartContainerRouter = SmartContainerRouter;
//# sourceMappingURL=SmartContainerRouter.js.map