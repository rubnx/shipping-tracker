...
class TrackingService {
    constructor() {
        this.CACHE_WARNING_AGE = 60; // 1 hour in minutes
        this.STALE_DATA_AGE = 1440; // 24 hours in minutes
        this.shipmentService = new ShipmentService_1.ShipmentService();
    }
    /**
     * Main tracking method with comprehensive error handling
     * Requirements: 5.3, 5.4, 5.5, 7.1, 7.4
     */
    async trackShipment(trackingNumber, trackingType, userSession, forceRefresh = false) {
     ...
     */
    async performFreshTracking(trackingNumber, trackingType, userSession) {
        try {
            const shipmentData = await this.shipmentService.trackShipment(trackingNumber, trackingType, userSession);
            return {
                success: true,
                data: shipmentData,
                fromCache: false,
     ...
     */
    async getCachedTrackingData(trackingNumber, trackingType) {
        try {
     ...
     */
    async getStaleTrackingData(trackingNumber, trackingType) {
        try {
