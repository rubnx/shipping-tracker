...
describe('TrackingService', () => {
    let trackingService;
    beforeEach(() => {
        trackingService = new TrackingService_1.TrackingService();
        jest.clearAllMocks();
    });
    describe('Input Validation', () => {
        ...
        it('should reject null tracking number', async () => {
            const result = await trackingService.trackShipment(null);
            expect(result.success).toBe(false);
    ...
    describe('Error Categorization', () => {
        it('should categorize network errors correctly', async () => {
            ...
            jest.spyOn(mockShipmentService, 'trackShipment').mockRejectedValue(new Error('ECONNREFUSED connection refused'));
        ...
        it('should categorize timeout errors correctly', async () => {
            const mockShipmentService = trackingService['shipmentService'];
            jest.spyOn(mockShipmentService, 'trackShipment').mockRejectedValue(new Error('Request timeout after 10000ms'));
            const result = await trackingService.trackShipment('TEST123');
            expect(result.success).toBe(false);
        ...
        it('should categorize rate limit errors correctly', async () => {
            const mockShipmentService = trackingService['shipmentService'];
            jest.spyOn(mockShipmentService, 'trackShipment').mockRejectedValue(new Error('Rate limit exceeded'));
            const result = await trackingService.trackShipment('TEST123');
            expect(result.success).toBe(false);
        ...
        it('should categorize tracking not found errors correctly', async () => {
            const mockShipmentService = trackingService['shipmentService'];
            jest.spyOn(mockShipmentService, 'trackShipment').mockRejectedValue('Unable to find tracking information. Please verify the tracking number and try again.');
        ...
        it('should categorize service unavailable errors correctly', async () => {
            const mockShipmentService = trackingService['shipmentService'];
            jest.spyOn(mockShipmentService, 'trackShipment').mockRejectedValue('Tracking services are temporarily unavailable. Please try again in a few minutes.');
        ...
        it('should handle unknown errors gracefully', async () => {
            const mockShipmentService = trackingService['shipmentService'];
            jest.spyOn(mockShipmentService, 'trackShipment').mockRejectedValue(new Error('Some unexpected error'));
    ...
    describe('Caching Behavior', () => {
        ...
        it('should return fresh cached data when available', async () => {
            ...
            jest.spyOn(mockShipmentService, 'transformCachedToShipmentData').mockReturnValue(mockShipmentData);
        ...
        it('should attempt refresh for old cached data', async () => {
            ...
            jest.spyOn(mockShipmentService, 'transformCachedToShipmentData').mockReturnValue(oldData);
        ...
        it('should fall back to cached data when refresh fails', async () => {
            ...
            jest.spyOn(mockShipmentService, 'trackShipment').mockRejectedValue(new Error('API temporarily unavailable'));
    ...
    describe('Tracking History', () => {
        ...
        it('should handle history retrieval errors', async () => {
            const mockShipmentService = trackingService['shipmentService'];
            jest.spyOn(mockShipmentService, 'trackShipment').mockRejectedValue(new Error('Service unavailable'));
    ...
    describe('Search Suggestions', () => {
        ...
        it('should handle search suggestion errors gracefully', async () => {
            const mockShipmentService = trackingService['shipmentService'];
            jest.spyOn(mockShipmentService, 'getSearchSuggestions').mockRejectedValue(new Error('Database error'));
...
//# sourceMappingURL=TrackingService.test.js.map